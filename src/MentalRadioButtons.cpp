...
MentalRadioButtonsWidget::MentalRadioButtonsWidget(MentalRadioButtons *module) : ModuleWidget(module)
{
  ...
  for (int i = 0; i < 7 ; i++)
  {  
    addInput(createPort<GateInPort>(Vec(3, top_space + spacing * i), PortWidget::INPUT, module, MentalRadioButtons::INS1 + i));
    addOutput(createPort<GateOutPort>(Vec(63, top_space + spacing * i), PortWidget::OUTPUT, module, MentalRadioButtons::OUTPUT + i));
    ...
    addChild(createLight<MedLight<BlueLED>>(Vec(40, top_space + 8 + spacing * i), module, MentalRadioButtons::BUTTON_LEDS + i));
	  ...
	  /// group 2
   addInput(createPort<GateInPort>(Vec(3, 10 + group_offset + spacing * i), PortWidget::INPUT, module, MentalRadioButtons::INS2 + i)); 
   addOutput(createPort<GateOutPort>(Vec(63, 10 + group_offset +  spacing * i), PortWidget::OUTPUT, module, MentalRadioButtons::BUTTON2_OUT + i));
